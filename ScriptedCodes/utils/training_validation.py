...
def save_model(model, saved_model_dir, fold_number, filename="best_metric_model.pth"):
    if isinstance(model, torch.nn.DataParallel):
        model = model.module
    torch.save(model.state_dict(), os.path.join(saved_model_dir, "Fold" + str(fold_number) + "_" + filename))
...
class Trainer:
    ...
    def train(self, global_step, train_loader, val_loader, dice_val_best, global_step_best, fold_number):
        self.model.train()
        epoch_loss = 0
        step = 0
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (global_step % self.eval_num == 0 and global_step != 0) or global_step == self.max_iterations:
                ...
                if dice_val > dice_val_best:
                    ...
                    save_model(self.model, self.saved_model_dir, fold_number, "best_metric_model.pth")
